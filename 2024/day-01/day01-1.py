...
number in the left list with the smallest number in the right list, then the second-smallest left number with the second-smallest 
...
Solution:
Use "map" and "zip" functions to solve the problem without loops.
Use "map" to split left and right columns into the lists and convert to integers at the same time.
Calculate final sum by running "zip" on two lists and summing up delta between two coordinates.
"""
...
def main() -> None:
    ...
    result = sum(map(lambda x: abs(x[0] - x[1]), zip(locations_left, locations_right)))
    print(result)
