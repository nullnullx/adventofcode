...
though - this is Island Island, after all! I bet the gardener would know. He's on a different island, 
though - er, the small kind surrounded by water, not the floating kind. We really need to come up 
with a better naming scheme. Tell you what: if you can help me with something quick, 
...
two lists of numbers separated by a vertical bar (|): a list of winning numbers and then a list of numbers 
...
in the list of winning numbers. The first match makes the card worth one point and each match 
...
for card in scratchcards:
    ...
    winning_numbers = len(winning_set.intersection(elf_set))
    total_points += 2 ** (winning_numbers - 1) if winning_numbers > 0 else 0
print(total_points)
