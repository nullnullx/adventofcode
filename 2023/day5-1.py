...
to use with each kind of seed, what type of fertilizer to use with each kind of soil, what type of water 
to use with each kind of fertilizer, and so on. Every type of seed, soil, fertilizer and so on is 
identified with a number, but numbers are reused by each category - that is, soil 123 and 
...
into numbers in a destination category. That is, the section that starts with seed-to-soil map: describes 
how to convert a seed number (the source) to a soil number (the destination). This lets the gardener and 
...
is the same length, but it starts at 50, so its two values are 50 and 51. With this information, 
...
Solution:
Parse almanac data into dict with all numbers for each category.
...
def main():
    ...
    locations = [find_location(seed, almanac_data) for seed in seeds]
    print(min(locations))
