...
for line in sys.stdin:
    ...
    for game_result in game_results.split(';'):
        for color_cubes in game_result.split(','):
            number_of_cubes, color_of_cubes = color_cubes.split()
            if int(number_of_cubes) > min_cubes[color_of_cubes]:
                min_cubes[color_of_cubes] = int(number_of_cubes) 
    all_sets_sum += min_cubes['red'] * min_cubes['green'] * min_cubes['blue']
    # all_sets_sum += reduce((lambda x, y: x * y), min_cubes.values())
print(all_sets_sum)
